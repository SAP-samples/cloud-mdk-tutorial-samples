...
export class MyMapClass extends IControl {
    ...
    public initialize(props: any): any {
        ...
        if (app.ios) {
            /*
            uncomment below code if you want to load google maps on ios devices.
            Initially Google Maps for iOS is loaded with sydney location, post the latitude and longitude is resolved, the maps camera
            is moved to right coordidates
            for more details on google maps for ios visit
            https://developers.google.com/maps/documentation/ios-sdk/intro
            GMSServices.provideAPIKey("Enter your API key here");
            this._mapView = GMSMapView.alloc().initWithFrameCamera(CGRectZero, this._createCameraPosition(-33.8683, 151.2086));*/
    ...
    private _createCameraPosition(lat,lon) {
        return GMSCameraPosition.cameraWithLatitudeLongitudeZoomBearingViewingAngle(lat, lon, 6, 10, 10);
    }
        ...
        .then((address) => {
            ...
            if (app.ios){
                return this.getlatlang(customerAddress)
                    .then((cordinates) => {   
                        /* uncomment below code for google maps
                        this._mapView.animateToCameraPosition(this._createCameraPosition(cordinates.latitiude, cordinates.longitude));
                        this._marker = GMSMarker.markerWithPosition(CLLocationCoordinate2DMake(cordinates.latitiude, cordinates.longitude));
                        this._marker.title = this._customerInfo.lastName + "'s" + " location";
